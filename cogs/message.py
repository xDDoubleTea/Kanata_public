...
import random
import json
from view.nick_cnl_msg_handling import *
...
class message(commands.Cog):
    ...
    async def on_message(self, message:discord.Message):
        ...
        #        pass
        #
        if '台中市立文華高級中學是一所積極新創、學科齊全、學術實力雄厚、辦學特色鮮明，在國際上具有重要影響力與競爭力的綜合性高中，在多個學術領域具有非常前瞻的科技實力，擁有世界一流的實驗室與師資力量，各種排名均位於全球前列。歡迎大家報考台中市立文華高中。' in ctx:
            return await message.delete()
        
        #if message.content.count('||') >= 2:
        #    msg = message.content.split('||')
        #    return await message.reply(mention_author = False, content = "訊息解碼：" + "".join(msg))

        if (ctx.startswith('$') and ctx.endswith('$')) or (ctx.startswith('```latex') and ctx.endswith('```')):
        ...
        if stuff["tmr"] == 1:
            if message.guild != None:
                if message.guild.id == 910150769624358914 and message.channel.type == discord.ChannelType.private_thread:
                    me = self.client.get_user(Me)
                    try:
                        await message.channel.fetch_member(Me)
                        return
                    except:
                        msg = 0
                        if len(message.stickers) != 0:
                            msg = await me.send(f'{message.author.display_name} 在 {message.channel.name}中說 ： \n{message.content}\n並傳送了貼圖:{message.stickers[0].url}')
                        else:
                            msg = await me.send(f'{message.author.display_name} 在 {message.channel.name}中說 ： \n{message.content}')
                        if len(message.attachments)>0:
                            for i in message.attachments:
                                file = await i.to_file()
                                msg = await msg.add_files(file)
                        if message.reference != None:
                            reference:Message = await message.channel.fetch_message(message.reference.message_id)
                            await msg.edit(content = f'{msg.content}\n回覆了{reference.author.mention}所傳送的訊息：\n{reference.content}')
    ...
    async def on_message_edit(self, before:discord.Message, after:discord.Message):
        ...
        if stuff["tmr"]:
            if before.guild != None:
                if before.guild.id == 910150769624358914 and before.channel.type == discord.ChannelType.private_thread and after != None:
                    message = after
                    me = self.client.get_user(Me)
                    msg = 0
                    if len(message.stickers) != 0:
                        msg = await me.send(f'{message.author.display_name} 在 {message.channel.name}中編輯了他的訊息 ： \n{message.content}並傳送了貼圖:{message.stickers[0].url}')
                    else:
                        msg = await me.send(f'{message.author.display_name} 在 {message.channel.name}中編輯了他的訊息 ： \n{message.content}')
                    if len(message.attachments)>0:
                        for i in message.attachments:
        ...
        #if after.content.count('||') >= 2:
        #    msg = after.content.split('||')
        #    return await after.reply(mention_author = False, content = "訊息解碼：" + "".join(msg))
