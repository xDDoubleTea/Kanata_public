...
from discord.ui import Modal, Button, View,button
first_words = ['習慣','今年','希望','食物','廁所','容易','以後','參加','暑假','請問','房間','下雨','走路','科學','冰箱','上午','小心','杯子','介紹','汽水','足球']
...
class word_chain(View):
    ...
    async def is_valid_input(self, new_word:str):
        if new_word[0] == self.words[len(self.words)-1][1][len(self.words[len(self.words)-1][1])-1] and not(new_word[0] == new_word[len(new_word)-1]):
